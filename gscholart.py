...
class Parser(object):
  ...
  """
  def parse(self, html, fieldset, max_results=20):
...
URL_META   = '/scholar?hl=en&btnG=Search&as_subj=eng&as_sdt=1,5&as_ylo=&as_vis=0&num=20'
URL_QUERY  = '&q={query}'
URL_START = '&start={start}'
URL_JOURNAL = '&as_publication={journal}'

def _format_url(query, journal='',start=0):
  str7 = (URL_ROOT + URL_META + URL_QUERY + (URL_JOURNAL if journal else '') + (URL_START if start else '')).format(
    query=quote(query), journal=quote(journal), start=quote(str(start)))
  print(str7)
  return str7
...
# ----------------------------------------------------------------------------
def query(search='', journal='', start='', max_results=20, fetcher=None, fieldset=Article):
  """
  query Google Scholar with a search phrase, and optional journal
  ...
  url = _format_url(search, journal,start)
...
if __name__ == '__main__':
  ...
  Retrieve article information from Google Scholar on the command line. e.g.
    ...
    scholar.py --max_results 1 --encoding json --file articles.json --journal Marr Theory of edge detection
  """
  import argparse

  # setup the parser
  parser = argparse.ArgumentParser(description='Retrieve article information from Google Scholar')
  parser.add_argument('-a', '--journal', help='journal name', default='')
  ...
  articles = query(search, args.journal, args.max_results)
